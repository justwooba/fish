...
const STEPS: Step[] = [
  { action: { kind: "popup" }, title: "Welcome to Fish!",
    message: "You're on Team A (blue) with Alice and Bob. Your opponents are Carol, Dave, and Eve (red). Players alternate around the table." },
  { action: { kind: "popup" }, title: "The Goal",
    message: "Score is 4–2 — you need one more set to win! Three sets remain: Low Hearts (2–7♥), High Diamonds (9–A♦), and High Clubs (9–A♣)." },
  { action: { kind: "popup" }, title: "Your Hand",
    message: "You have 3♥, 5♥, and 10♦. You can ask opponents for cards from sets you hold cards in — Low Hearts and High Diamonds." },
  { action: { kind: "popup" }, title: "How Asking Works",
    message: "Pick an opponent and ask for a specific card. You must hold a card from the same set. If they have it, it's yours and you go again. If not, the turn passes to them." },
  { action: { kind: "ask", target: "carol", card: "9:diamonds" }, title: "Ask Carol for 9♦",
    message: "You hold 10♦, so you can ask for High Diamonds. Click Carol, then ask for the 9 of Diamonds.",
    highlight: "carol" },
  { action: { kind: "popup" }, title: "Got it!",
    message: "Carol had the 9♦! Since your ask succeeded, it's still your turn." },
  { action: { kind: "ask", target: "carol", card: "J:diamonds" }, title: "Ask Carol for J♦",
    message: "Let's see if Carol has more diamonds. Ask her for the Jack of Diamonds.",
    highlight: "carol" },
  { action: { kind: "popup" }, title: "Miss!",
    message: "Carol doesn't have the J♦. The turn passes to her — the opponent you asked." },
  { action: { kind: "popup" }, title: "Carol Declares High Clubs",
    message: "Carol's team has been collecting clubs. She declares High Clubs — assigning each card to the correct teammate. If she's right, Team B scores." },
  { action: { kind: "opponent_declare", declarer: "carol", setId: "high_clubs",
    assignments: { carol: ["9:clubs", "10:clubs", "J:clubs"], dave: ["Q:clubs", "K:clubs"], eve: ["A:clubs"] }, team: "B" },
    title: "Team B Scores!",
    message: "Carol's declaration was correct. Score is now 4–3. After scoring, the winning team picks who goes next." },
  { action: { kind: "choose_turn", chosen: "dave" }, title: "Dave's Turn",
    message: "Team B chose Dave. He's been collecting diamonds — watch out!" },
  { action: { kind: "opponent_ask", asker: "dave", target: "you", card: "10:diamonds", success: true },
    title: "Dave Takes Your 10♦",
    message: "Dave asked you for 10♦ and you had it. When someone asks YOU, you must give the card if you have it." },
  { action: { kind: "opponent_ask", asker: "dave", target: "you", card: "9:diamonds", success: true },
    title: "Dave Takes Your 9♦ Too",
    message: "Dave keeps going! This is the back-and-forth of Fish — when two players have cards in the same set, they trade asks." },
  { action: { kind: "opponent_ask", asker: "dave", target: "you", card: "J:diamonds", success: false },
    title: "Dave Misses!",
    message: "Dave asked for J♦ but you don't have it (Alice does!). Key insight: Dave's ask tells you he has diamonds but NOT the J♦. Every ask reveals information!" },
  { action: { kind: "popup" }, title: "Your Options",
    message: "You only have 3♥ and 5♥ now — both Low Hearts. You can only ask from sets you hold cards in, so it's time to go after hearts." },
  { action: { kind: "popup" }, title: "Strategy Tip",
    message: "Keep asking the same opponent while they have cards in your set. If you switch too early, they can take cards back on their turn." },
  { action: { kind: "ask", target: "dave", card: "2:hearts" }, title: "Ask Dave for 2♥",
    message: "Dave has hearts. Ask him for the 2 of Hearts.",
    highlight: "dave" },
  { action: { kind: "popup" }, title: "Nice!",
    message: "Keep pressing Dave for hearts!" },
  { action: { kind: "ask", target: "dave", card: "4:hearts" }, title: "Ask Dave for 4♥",
    message: "Ask Dave for the 4 of Hearts.",
    highlight: "dave" },
  { action: { kind: "popup" }, title: "The 50/50",
    message: "You have 2♥, 3♥, 4♥, 5♥. The remaining hearts are 6♥ and 7♥. Dave's remaining cards are a mix of hearts and diamonds. Does he have 6♥, 7♥, or neither?" },
  { action: { kind: "popup" }, title: "Deduction",
    message: "Dave started with hearts and clubs. His clubs are gone (declared). He still has some hearts and diamonds. You need to guess — does he have the 6♥ or the 7♥? Let's try 6♥." },
  { action: { kind: "ask", target: "dave", card: "6:hearts" }, title: "Ask Dave for 6♥",
    message: "The 50/50 guess — ask Dave for the 6 of Hearts.",
    highlight: "dave" },
  { action: { kind: "popup" }, title: "Correct Guess!",
    message: "Dave had the 6♥! You now have 5 of 6 Low Hearts. Dave only has diamonds left, so the 7♥ must be with your teammate Alice." },
  { action: { kind: "popup" }, title: "How Declaring Works",
    message: "Assign each card in the set to the teammate who holds it. Get them all right = your team scores. Get one wrong = misdeclare. You can't take it back!" },
  { action: { kind: "declare", setId: "low_hearts" }, title: "Declare Low Hearts",
    message: "Assign your 5 hearts to yourself and the 7♥ to Alice." },
  { action: { kind: "end" }, title: "You Win! 🎉",
    message: "Team A scores Low Hearts — 5–3! You've learned asking, failed asks, reading information, back-and-forth, 50/50s, and declaring. Go play for real!" },
];
...
export default function TutorialClient() {
  ...
  // For ask steps — get target and card info
  ...
  return (
    <main className="min-h-dvh flex flex-col items-center px-4 py-6 relative">
      ...
      <div className="w-full max-w-2xl flex flex-col gap-5 mt-10">
        ...
        {/* Inline popup — small card at top, doesn't cover anything */}
        {showPopup && cur && (
          ...
          {SEATS.map(({ pos, idx }) => {
            ...
            return (
              <div key={p.id} className={pos}>
                <div className={`relative flex flex-col items-center gap-1 p-3 rounded-xl border transition-all duration-200 min-w-[80px]
        ...
        {/* Ask UI — locked to the specific card, no free selection */}
        {!showPopup && isAskStep && askTarget && askCard && askCardSet && (
          <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5 space-y-4">
            ...
            {/* Show the set visually — your cards dimmed, target card highlighted */}
            <div className="flex flex-wrap gap-1.5">
              {getCardKeysInSet(askCardSet).map(ck => {
                ...
                if (isTarget) return (
                  <div key={ck} className={`w-12 h-[68px] rounded-lg border-2 border-solid border-blue-400 bg-blue-500/20 ring-1 ring-blue-400/30 scale-105 flex flex-col items-center justify-center text-xs font-semibold text-blue-300`}>
                    {isJ ? <span className="text-[8px] font-bold uppercase">JKR</span> : <><span className="leading-none text-sm">{rank}</span><span className="leading-none text-lg mt-0.5">{sym}</span></>}
                  </div>
                );

                // Other missing cards — shown as faint ghosts, not clickable
                return (
                  <div key={ck} className={`w-12 h-[68px] rounded-lg border-2 border-dashed border-white/[0.08] flex flex-col items-center justify-center text-xs font-semibold ${col} opacity-20`}>
                    {isJ ? <span className="text-[8px] font-bold uppercase">JKR</span> : <><span className="leading-none text-sm">{rank}</span><span className="leading-none text-lg mt-0.5">{sym}</span></>}
                  </div>
                );
              })}
